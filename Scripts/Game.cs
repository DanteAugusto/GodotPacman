...
public partial class Game : Node2D
{
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		GD.Print($"GyroX={gyroX}");
		GD.Print($"GyroY={gyroY}");
		// toggle fullscreen
	...
	private void ReadArduino()
	{
		if (serialPort == null)
		{
			GD.PrintErr("serialPort é NULL");
			return;
		}

		if (!serialPort.IsOpen)
		{
			GD.PrintErr("serialPort não está aberta");
			return;
		}
		...
		try
		{
			...
			roll = float.Parse(values[0], System.Globalization.CultureInfo.InvariantCulture);
			pitch = float.Parse(values[1], System.Globalization.CultureInfo.InvariantCulture);
			gyroX = float.Parse(values[2], System.Globalization.CultureInfo.InvariantCulture);
			gyroY = float.Parse(values[3], System.Globalization.CultureInfo.InvariantCulture);
		}
